...
function httpGet(url, timeoutMs = 3000) {
  return new Promise((resolve) => {
    ...
    } catch (e) { clearTimeout(timer); resolve({ err: e.message, statusCode: null, body: '', headers: {} }); }
  });
}
...
function getBindingInfo(port) {
  try {
    ...
    const out = execSync(
      `lsof -iTCP:${port} -sTCP:LISTEN -nP 2>/dev/null || ss -tlnp 2>/dev/null | grep :${port}`,
      { stdio: ['ignore', 'pipe', 'ignore'], timeout: 5000 }
    ).toString();
    if (!out.trim()) return null;
    if (out.includes('0.0.0.0') || out.includes('*:')) return '0.0.0.0';
    if (out.includes('127.0.0.1') || out.includes('localhost') || out.includes('::1')) return '127.0.0.1';
    return 'unknown';
  } catch { return null; }
}
...
function extractModelName(body) {
  try {
    ...
    if (json.data?.length > 0) return json.data.map(m => m.id).filter(Boolean).slice(0, 3).join(', ');
