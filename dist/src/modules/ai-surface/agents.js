...
function getFullProcessList() {
    try {
        ...
        return execSync('ps -ewwo pid,user,args 2>/dev/null || ps aux 2>/dev/null', { stdio: ['ignore', 'pipe', 'ignore'], timeout: 8000 }).toString();
    }
    catch {
        return '';
    }
...
function hasNetworkConnections(pid) {
    try {
        const out = execSync(`lsof -p ${pid} -i -n -P 2>/dev/null | grep -v LISTEN`, { stdio: ['ignore', 'pipe', 'ignore'], timeout: 5000 }).toString();
...
function hasSensitiveDirAccess(pid) {
    try {
        const out = execSync(`lsof -p ${pid} 2>/dev/null`, { stdio: ['ignore', 'pipe', 'ignore'], timeout: 5000 }).toString();
